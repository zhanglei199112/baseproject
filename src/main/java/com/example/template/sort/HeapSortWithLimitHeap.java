...
public class HeapSortWithLimitHeap {
...
//          print(heap);
  ...
  private static void v1() {
    Long startTime = System.currentTimeMillis();
    int[] origin = initData();
//    int[] origin = {1,1,55,55,22,22,66,66,44,0,1,2,5,3,4};
    heapBuild(origin);
    //构建完成之后，替换最大的到末尾，然后重新进行排序
    heapSort(origin,origin.length-1);
    System.out.println("-----------------------");
//    print(heap);
    for(int i =0;i<10;i++){
  ...
  static int[] initData(){
    ...
    for(int i=0;i<10000000;i++){
      if(i%2==0){
//        oushu[oushuindex++] = random.nextInt(10000000);
        oushu[oushuindex++] = i;
      }
      if(i%2==1){
//        jishu[jishuindex++] = random.nextInt(10000000);
        jishu[jishuindex++] = i;
      }
    }
    //倒序
  ...
  private static int[] reverse(int[] oushu) {
    int[] result = new int[oushu.length];
    for (int i=0;i<oushu.length;i++){
      result[i] = oushu[oushu.length-1-i];
    }
    return result;
  }
